...
void Mesh::Create(const Vertex* vertices, uint32_t vertex_count, const uint32_t* indices, uint32_t index_count)
{
    ...
    glVertexAttribPointer(2, 4, GL_FLOAT, GL_TRUE, sizeof(Vertex), (const void*) offsetof(Vertex, color));
...
void Mesh::Draw() {
    ...
    glBlendFunc(shader->blend_src_factor, shader->blend_dst_factor);
...
std::optional<uint32_t> FindMeshIdx(aiNode* node) {
  if (node->mNumMeshes > 0) { return node->mMeshes[0]; }

  for (uint32_t i = 0; i < node->mNumChildren; ++i) {
    std::optional<uint32_t> mesh = FindMeshIdx(node->mChildren[i]);

    if (mesh.has_value()) { return mesh; }
  }
...
std::shared_ptr<Mesh> ImportMesh(const char* filepath) {
    ...
    std::optional<uint32_t> mesh_idx = FindMeshIdx(scene->mRootNode);
    assert(mesh_idx.has_value());

    aiMesh* mesh = scene->mMeshes[mesh_idx.value()];
    ...
    std::shared_ptr<Mesh> result_mesh{std::make_shared<Mesh>()};
    result_mesh->Create(vertices.data(), vertices.size(), indices.data(), indices.size());
    ...
    // stbi_uc* pixels = stbi_load(texture_filename.c_str(), &tex_width, &tex_height, &tex_channels, STBI_rgb_alpha);
    // assert(pixels);
    ...
    // result_mesh->texture = device->CreateTexture(tex_specification, pixels);

    // SamplerSpecification sampler_specification{};
    // sampler_specification.min_filter         = SamplerFilter::kLinear;
    // sampler_specification.mag_filter         = SamplerFilter::kLinear;
    // sampler_specification.anisotropy_enabled = true;
    // sampler_specification.max_anisotropy     = anisotropy;
    // sampler_specification.max_lod            = static_cast<float>(tex_mip_levels);

    // result_mesh->sampler = device->CreateSampler(sampler_specification);

    return result_mesh;
}

std::shared_ptr<Mesh> CreateTorusMesh(float outer_radius, float inner_radius, uint32_t outer_samples, uint32_t inner_samples) {
    ...
    return std::make_shared<Mesh>(torus_mesh);
}
...
std::shared_ptr<Mesh> CreateOctahedronMesh(uint32_t subdivisions) {
    ...
    return std::make_shared<Mesh>(octahedron_mesh);
}